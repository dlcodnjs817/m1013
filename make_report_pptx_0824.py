...
# ═══════════ 13. 왜 카메라를 설치했나 ═══════════
s = slide()
bar(s, "왜 카메라를 설치했나 — 목적과 검사 가능 범위", "08 목적")
tf = textbox(s, Cm(0.9), Cm(2.4), Cm(32), Cm(4.2))
bullets(tf, [
    (0, "① 정책 관측 확보 — ACT 정책의 입력은 손목캠·정면캠 이미지 2장. sim 안에서 정책을 구동하려면 "
        "매 스텝 관측 화면을 렌더로 생성해야 함", {"size": 14.5, "bold": True}),
    (1, "기존 검증(open-loop 평가 · 정책 궤적 물리 재생)은 모두 실제 OMX 영상을 먹인 것 — "
        "정책이 sim 을 본 적은 한 번도 없었음. 지금까지 시뮬레이션은 '재생 장치'였음", {"size": 13}),
    (0, "② 실물 준비 자산 확보 — 확정 파라미터·탐색 도구는 실물 손목캠 부품/마운트 선정 시 "
        "'해당 FOV·위치에서 시야가 충분한가'를 프린팅 전에 검증하는 데 재사용", {"size": 14.5, "bold": True}),
    (1, "관측 → 추론 → 관절명령 파이프라인 구조는 실물에서도 동일 — 도착 후 시간 절약", {"size": 13}),
])
rows = [
    ["시점", "이미지 출처", "로봇", "무엇을 알 수 있나"],
    ["지금까지 (STEP 4)", "실제 OMX 영상 파일", "없음 (예측만 비교)",
     "정책이 학습 분포를 재현하는가 (open-loop 0.62~0.67°)"],
    ["STEP 5-3 (가능해짐)", ("Isaac 렌더 ← 오늘 작업", {"bold": True}), "Isaac 안 가상 M1013",
     ("예상 못 한 상황에 반응하는가 — 큐브 위치 변경 · 파지 어긋남 복구 · 초기자세 교란", {"bold": True})],
    ["로봇 도착 후", "실물 카메라 사진", "진짜 M1013", "실제 성능 (Isaac 은 루프에서 빠짐)"],
]
table(s, Cm(0.9), Cm(7.2), Cm(32.1), rows, widths=[3.4, 4.4, 3.8, 10.4], size=12, rh=Cm(1.4))
tf = textbox(s, Cm(0.9), Cm(13.2), Cm(32), Cm(4.4))
bullets(tf, [
    (0, "한계 — 결론이 비대칭임을 명시할 것:", {"size": 14, "bold": True, "color": ORANGE}),
    (1, "sim 에서 성공해도 실물 성공의 보장이 아님 (렌더 화면 ≠ 실물 사진)", {"size": 13}),
    (1, "sim 에서 실패해도 정책 결함인지 sim 이 안 닮은 탓인지 구분 불가", {"size": 13}),
    (1, "따라서 closed-loop 성공률 숫자 자체의 증명력은 약함 — 가치는 '파이프라인 리허설'과 "
        "'반응성 관찰'에 있음", {"size": 13, "bold": True}),
])
foot(s)

# ═══════════ 14. 가상 카메라 vs 실물 카메라 ═══════════
s = slide()
bar(s, "가상 카메라와 실물 카메라는 어떻게 이어지나", "09 실물 연결")
tf = textbox(s, Cm(0.9), Cm(2.3), Cm(32), Cm(0.8))
run = tf.paragraphs[0].add_run()
run.text = "정책에는 '640×480 이미지를 꽂는 구멍'이 2개 — 무엇을 꽂든 정책은 구분하지 않음"
_set(run, 15, True, INK)
# 지금
caption(s, Cm(0.9), Cm(3.2), Cm(6), "지금 (로봇 없음)", 13, True, NAVY)
row1 = [("Isaac 렌더 이미지\n+ 관절 상태", BGSOFT, INK), ("ACT 정책", NAVY, WHITE),
        ("관절 명령", BGSOFT, INK), ("Isaac 안 가상 M1013", BGSOFT, INK)]
bw, bh, gap = Cm(6.6), Cm(1.8), Cm(0.9)
for i, (txt, bg, fg) in enumerate(row1):
    x = Cm(0.9) + i * (bw + gap)
    flowbox(s, x, Cm(4.0), bw, bh, txt, fill=bg, fg=fg, size=12.5)
    if i < 3:
        arrow(s, x + bw + Cm(0.1), Cm(4.0) + Cm(0.65))
# 도착 후
caption(s, Cm(0.9), Cm(6.4), Cm(18), "로봇 도착 후 (9/20~) — Isaac 은 루프에서 빠짐", 13, True, GREEN)
row2 = [("실물 카메라 사진\n+ 실물 관절 상태", BGSOFT, INK), ("ACT 정책", NAVY, WHITE),
        ("관절 명령 (ROS2 dsr)", BGSOFT, INK), ("진짜 M1013", GREEN, WHITE)]
for i, (txt, bg, fg) in enumerate(row2):
    x = Cm(0.9) + i * (bw + gap)
    flowbox(s, x, Cm(7.2), bw, bh, txt, fill=bg, fg=fg, size=12.5)
    if i < 3:
        arrow(s, x + bw + Cm(0.1), Cm(7.2) + Cm(0.65))
tf = textbox(s, Cm(0.9), Cm(9.5), Cm(32), Cm(8))
bullets(tf, [
    (0, "가상 카메라가 실물 카메라로 '변신'하는 것이 아님 — 같은 자리에 번갈아 꽂히는 서로 다른 두 장치", {"size": 14.5, "bold": True}),
    (0, "오늘 값의 두 가지 쓰임:", {"size": 14, "bold": True}),
    (1, "㉠ 실물 마운트 설계의 출발점 — 단, 그대로 복사는 불가", {"size": 13, "bold": True}),
    (2, "FOV 는 마운트가 아니라 카메라 부품이 결정 (오늘 60°는 절충값) → 부품 확정 후 위치 재계산", {"size": 12.5}),
    (2, "기구 제약 우선 — LEHR 본체·케이블 회피, 파지 시 테이블 충돌 없어야", {"size": 12.5}),
    (2, "OMX 화면의 완전 재현은 애초 불가 (M1013 그리퍼가 길어 실물 대비 약 70% 크기)", {"size": 12.5}),
    (1, "㉡ 설치 후 실측값으로 sim 갱신 → 이후에도 실물 대신 위험한 시도를 먼저 해보는 연습장으로 유지", {"size": 13, "bold": True}),
    (0, "권장 순서:  카메라 부품 결정 → 기구적으로 가능한 마운트 설계 → sim 에서 시야 검증(프린팅 전) → "
        "설치 후 실측 → JSON 갱신", {"size": 13.5, "bold": True, "color": GREEN}),
])
foot(s)

# ═══════════ 15. 방법 재평가 ═══════════
s = slide()
bar(s, "방법 재평가 — 다시 정밀도가 필요해지면 이 경로로", "10 재평가")
tf = textbox(s, Cm(0.9), Cm(2.4), Cm(32), Cm(1.4))
bullets(tf, [
    (0, "오늘 방식(겉보기 정합)은 상황상 합리적이었으나 교과서적 방법은 아니었음 — 아래는 검토 후 확인한 더 나은 접근", {"size": 14, "bold": True}),
])
rows = [
    ["방법", "내용", "평가"],
    ["실물 리그 실측",
     "OMX 정면캠 위치를 베이스 기준으로 자로 재고, 웹캠 모델명으로 FOV 스펙 확인",
     ("가장 싸고 확실 — 3라운드 탐색을 5분에 대체 가능했음 (미제안은 실수)", {"color": RED, "bold": True})],
    ["정면캠 solvePnP",
     "에피소드별 큐브 파지 위치(3D, 그리퍼 닫힘 시점 EE) ↔ 영상 속 큐브 픽셀(2D) 159쌍으로 포즈 복원",
     ("★ 타깃이 없던 게 아니라 큐브가 곧 타깃이었음. 비스듬한 시선이라 초점거리 추정 조건도 양호", {"bold": True})],
    ["손목캠 hand-eye",
     "FK 플랜지 포즈 + 큐브 관측의 재투영 오차 최소화로 '플랜지→카메라' 변환 유도",
     ("표준 알고리즘(cv2.calibrateHandEye 계열). 이 방식이면 +55° 경험적 보정 자체가 불필요", {"bold": True})],
    ["정책 액션 오차 지표",
     "GT 궤적 재생 → 렌더 관측을 정책에 투입 → 실제 영상 투입 시 예측과 비교",
     "픽셀 눈대중 대신 '목적에 직접 맞는' 채점. 제어 루프 없이 반나절 — 다만 실물 갭 예측력은 낮음"],
    ["카메라 포즈 랜덤화",
     "재렌더 시 카메라 포즈를 ±범위로 흔들어 학습",
     "sim2real 표준 관행 — 잔여 오정합에 정책이 둔감해짐 (co-training 채택 시 함께 적용)"],
]
table(s, Cm(0.9), Cm(4.0), Cm(32.1), rows, widths=[3.6, 8.4, 10.0], size=11.5, rh=Cm(1.9))
tf = textbox(s, Cm(0.9), Cm(15.6), Cm(32), Cm(2.4))
bullets(tf, [
    (0, "정정 사항: 6장에서 PnP 를 '캘리브레이션 타깃 없음'으로 기각했으나 오판 — 데이터셋에 이미 159쌍의 "
        "3D↔2D 대응이 존재했음", {"size": 13, "color": RED, "bold": True}),
    (0, "단, 이 정밀 경로는 v7 이어학습 여부에 따라 필요성이 달라짐 (다음 장) — 실물 마운트 설계 시점에 재검토", {"size": 13}),
])
foot(s)

# ═══════════ 16. 범위 조정 + 4주 계획 ═══════════
s = slide()
bar(s, "범위 조정과 4주 준비 계획 — 실물 도착 2026-09-20", "11 계획")
tf = textbox(s, Cm(0.9), Cm(2.4), Cm(32), Cm(3.0))
bullets(tf, [
    (0, "범위 조정 근거 — v7 실기 수집(30~50ep) · 이어학습 경로가 확정됨:", {"size": 14.5, "bold": True}),
    (1, "새 데이터가 새 카메라 시점으로 수집되므로 'OMX 화면 정밀 재현'의 가치 하락 → 마운트는 "
        "기구적으로 튼튼하고 시야 좋은 자리면 충분", {"size": 13}),
    (1, "sim 도메인 갭은 실물 갭의 예측력이 낮음 (렌더와 실물 사진은 서로 다른 방식으로 OMX 영상과 다름) "
        "→ STEP 5-3 · 5-4 우선순위 하향", {"size": 13, "color": ORANGE, "bold": True}),
    (0, "원칙: 도착 후에만 할 수 있는 일을 최소화 — 도착 당일부터 v6 궤적 실행 + v7 수집이 돌아가야 함", {"size": 14, "bold": True}),
])
rows = [
    ["주차", "핵심 작업", "비고"],
    ["1주 (8/25~29)", ("LEHR 모델 확정·실측 → TCP 상수 확정 → 재변환(1분)+재학습(~3h) / 카메라 부품 선정·발주", {"bold": True}),
     ("★ 최우선 병목", {"color": RED, "bold": True})],
    ["2주 (9/1~5)", "손목캠 마운트 + 그리퍼 어태치먼트(일자·연질 팁) 설계·프린팅 / doosan-robot2 가상 모드로 ROS2 환경 선구축",
     "카메라 실물 도착 후 치수 확정"],
    ["3주 (9/8~12)", ("v7 수집 스크립트 (LeRobot 포맷 · 카메라 2대+관절 30Hz 동기) / v6 궤적 실행 스크립트", {"bold": True}),
     "도착 당일 쓸 도구 · 가상 모드로 검증"],
    ["4주 (9/15~19)", "통합 리허설 · 도착 당일 절차서 · 버퍼", "Isaac 을 실물 대역으로 사용"],
]
table(s, Cm(0.9), Cm(6.0), Cm(32.1), rows, widths=[2.8, 14.4, 4.4], size=12, rh=Cm(1.5))
tf = textbox(s, Cm(0.9), Cm(13.8), Cm(15.6), Cm(4.4))
bullets(tf, [
    (0, "LEHR 확정이 최우선인 이유 — 3가지를 동시에 막고 있음:", {"size": 13.5, "bold": True, "color": RED}),
    (1, "TCP 0.12m 잠정값 확정 불가 (→ 데이터셋 미확정)", {"size": 12.5}),
    (1, "그리퍼 어태치먼트 설계 불가", {"size": 12.5}),
    (1, "손목캠 마운트 설계 불가 (카메라가 그리퍼를 피해야 함)", {"size": 12.5}),
])
tf = textbox(s, Cm(17.2), Cm(13.8), Cm(15.8), Cm(4.4))
bullets(tf, [
    (0, "카메라 선정 기준:", {"size": 13.5, "bold": True}),
    (1, "FOV 60~70° 대역 · 소형(그리퍼 옆 장착) · USB UVC(기존 수집 코드 재사용)", {"size": 12.5}),
    (1, "정면캠은 OMX 리그의 것을 그대로 옮겨 쓰는 것이 최선 — 같은 카메라면 화질·색감·왜곡이 일치해 "
        "도메인 갭이 줄어듦", {"size": 12.5, "bold": True, "color": GREEN}),
])
foot(s)

# ═══════════ 14. 링크·참조 ═══════════
s = slide()
bar(s, "링크 · 참조 경로", "12 참조")
rows = [
    ["자원", "위치 / URL"],
