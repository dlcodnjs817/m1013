...
# ★ 2026-09-09 정정: 0.002 → 0.007.
#   JAW_CLEAR 는 「큐브가 조 끝 아래로 삐져나오는 양」이자 「조 끝의 상판 여유」다.
#   기하 하한은 0.005 — 큐브 35 중 조 사이에 들어갈 수 있는 높이가 30(= 판 밑면 z53 ~ 조 끝 z83)
#   뿐이라, 그보다 작으면 큐브 윗면이 핑거 마운트 판을 파고들어야 해서 물리적으로 불가능하다.
#   (0.002 이면 3 mm 관통 — 폼이라 눌려서 동작은 하지만 파지 깊이가 매번 달라지고 놓기도 3 mm 낮다)
#   하한 0.005 는 여유가 정확히 0 이라 큐브 높이 편차·상판 높이 오차를 못 흡수한다.
#   +2 mm 를 더해 0.007 로 잡는다 → TCP 0.0725, 조 물림 28/35 mm (80%), 조 끝 상판 여유 7 mm.
JAW_CLEAR = 0.007    # 파지 시 조 끝이 상판 위로 띄우는 여유
TCP = JAW_TIP + JAW_CLEAR - CUBE_SIZE / 2   # = 0.0725
ANCHOR_K = 5
SMOOTH_W = 7
NUDGE_DEG = 3.0
